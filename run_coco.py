...
def mkcityjson(command):
    ...
    city = {
        ...
        "is_elasticity": True,
        "start_cmd": command,
        "init_cmd": "pip3 install -r requirements.txt && pip3 install -e . && cp -r /apdcephfs/private_inchzhang/pretrained /dev/shm/ && ln -s /dev/shm/pretrained && mkdir /dev/shm/coco_stuff10k && cp -r /youtu-public/cityscapes/leftImg8bit /dev/shm/cityscapes/ &&  cp -r /youtu-public/cityscapes/gtFine /dev/shm/cityscapes/ && ln -s  /dev/shm/cityscapes data/cityscapes && python3 tools/convert_datasets/cityscapes.py data/cityscapes",
        "exit_cmd": "",
        "enable_evicted_pulled_up": True,
        "enable_evicted_end_task": True,
        "cuda_version": "10.1",
        "dataset_id": "3981D6265BB241F89B128419F0481E87	"
    }
    return city


lines = open('local_configs/train_10.26.sh','r').read().splitlines()
print(lines)
for i in lines:
